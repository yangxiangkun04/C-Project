...
//增量同步：适用于数据量很大，表中数据只插入，没有修改和删除,需要表中有自增字段
//1.从本地表中查询已同步记录的最大id
//2.从远程表中查询需要同步的记录，用rowid效率更高
//3.把需要同步的记录分批插入本地表
...
//remotecols与localcols中如果有不同名的字段，就应该在输入xml参数中显式的指定字段映射。
//主函数中，若二者有为空的话就会用localtname表中的全部来填充，使得同步发生错误
...
void _help(char *argv[])
{
    ...
    printf("把T_ZHOBTMIND1@db138表中全部的记录增量同步到T_ZHOBTMIND2中。\n");
    printf("Sample:/project/tools/bin/procctl 10 /project/tools/bin/syncinc /log/idc/syncinc_ZHOBTMIND2.log "\
              "\"<localconnstr>idc/idcpwd@snorcl11g_138</localconnstr><remoteconnstr>idc/idcpwd@snorcl11g_138</remoteconnstr>"\
              "<charset>AMERICAN_AMERICA.AL32UTF8</charset>"\
              "<remotetname>T_ZHOBTMIND1</remotetname><lnktname>T_ZHOBTMIND1@db138</lnktname>"\
    ...
    printf("把T_ZHOBTMIND1@db138表中满足\"and obtid like '57%%'\"的记录增量同步到T_ZHOBTMIND3中。\n");
...
bool _syncinc(bool &bcontinue)
{
    ...
    // insert into T_ZHOBTMIND3(obtid,ddatetime,t,p,u,wd,wf,r,vis,uptime,keyid)
    //                   select obtid,ddatetime,t,p,u,wd,wf,r,vis,uptime,keyid from T_ZHOBTMIND1@db128
