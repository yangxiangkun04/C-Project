...
//增量抽取：按查询条件抽取表中新增的数据(表中的数据只会新增，不会修改),需要绑定一个输入参数：上次已抽取数据的增量字段的最大值
...
void _help()
{
    ...
    printf("       /project/tools/bin/procctl   30 /project/tools/bin/dminingoracle /log/idc/dminingoracle_ZHOBTMIND.log "\
              ...
              "<maxcount>1000</maxcount><connstr1>scott/123456@snorcl11g_138</connstr1>\"\n\n");
...
bool writeincfield()
{
     ...
     if (strlen(starg.connstr1)!=0)
    {
        ...
        sqlstatement stmt(&conn1);
        stmt.prepare("update T_MAXINCVALUE set maxincvalue=:1 where pname=:2");  //准备更新表的sql语句
