...
int main(int argc,char* argv[])
{
    ...
    if(conn.connecttodb(starg.connstr,"AMERICAN_AMERICA.AL32UTF8")!=0)  //不启用自动提交
    {
...
bool _migratetable()
{
    ...
    //查询到的数据有很多行
    char keyvalues[starg.maxcount][21];  //存放唯一键字段的值的数组,每一行数据的唯一键值，作为绑定参数传入stmtdel语句
    ...
    while(true)
    {
        ...
        {
            if(stmtins.execute()!=0)  //执行插入数据打sql语句
            {
    ...
    if(stmtsel.rpc()>0) //如果是空表，rpc为0，不会写入日志
        logfile.write("migrate %s to %s %drows in %.02fsec.\n",starg.tname,starg.totname,stmtsel.rpc(),timer.elapsed());
