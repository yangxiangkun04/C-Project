...
//数据入库参数的结构体
//数据要入库的话肯定需要一些参数，比如xml文件的匹配规则，将数据入库到哪个表等
...
//在处理xml文件之前，如果strxmltotable.execsql不为空，就执行它
bool execsql();
...
int main(int argc,char *argv[])
{
    ...
    {
        printf("打开日志文件失败（%s）.n",argv[1]); return -1;
    }
...
void _help(char *argv[])
{
    ...
    printf("Sample:/project/tools/bin/procctl 10 /project/tools/bin/xmltodb /log/idc/xmltodb_vip.log "\
              "\"<connstr>idc/idcpwd@snorcl11g_138</connstr><charset>AMERICAN_AMERICA.AL32UTF8</charset>"\
...
bool _xmltodb()
{
    ...
    int icout=50;    // 循环的计数器，初始化为50是为了第一次进入循环的时候就加载参数，用于控制多久重新加载一次参数配置文件。
    ...
    while (true)      // 每循环一次，执行一次入库任务。
    {
        ...
        while (true)  // 遍历所有的xml文件，一个一个用_xmltodb()子函数去处理，然后根据ret结果分类。
        {
            // 读取目录，得到一个xml文件。
...
// 处理xml文件的子函数，返回值：0-成功，其它的都是失败，失败的情况有很多种，暂时不确定。
// 总体流程：接收一个 XML 文件路径 → 解析 XML → 映射到数据库表 → 执行插入/更新 → 记录处理情况并返回状态码
int _xmltodb(const string &fullfilename,const string &filename)
{
    timer.start();          // 开始计时。
    ...
    // 在处理xml文件之前，如果stxmltotable.execsql不为空，就执行它，预处理sql。
