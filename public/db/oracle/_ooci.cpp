...
int connection::connecttodb(const string &connstr,const string &charset,bool autocommitopt)
{
    ...
    printf("user=[%s], pass=[%s], tns=[%s]\n", m_env.user, m_env.pass, m_env.tnsname);
    ...
    if ( oci_ret != OCI_SUCCESS && oci_ret != OCI_SUCCESS_WITH_INFO )
    {
        oci_close(&m_env); m_cda.rc=1017; strncpy(m_cda.message,"ORA-01017: invalid username/password,login denied.\n",128); return -1;
    }
