...
int main(int argc,char *argv[])
{
    ...
    // pollfd是一个结构体数组
    fds[listensock].fd=listensock;        //第一个参数表示要监视的socket
    fds[listensock].events=POLLIN;        //第二个参数表示监视对第一个参数进行的事件 POLLIN表示读事件，POLLOUT表示写事件。
    ...
    while (true)        // 事件循环。
    {
        ...
        for (int eventfd=0;eventfd<=maxfd;eventfd++)
        {
            ...
            {
                // 如果是客户端连接的socket有事件，表示有报文发过来了或者连接已断开。
